...
void compileForSt(void) {
  ...
  Type* forType = checkDeclaredVariable(currentToken->string)->varAttrs->type; // --- ADDED: capture loop var type ---

  eat(SB_ASSIGN);
  Type* fromType = compileExpression();
  checkTypeEquality(forType, fromType);

  eat(KW_TO);
  Type* toType = compileExpression();
  checkTypeEquality(forType, toType);
...
void compileArguments(ObjectNode* paramList) {
  ...
  case SB_LPAR:
    eat(SB_LPAR);
    // --- ADDED: iterate parameters and check each argument ---
    if (paramList == NULL)
  ...
  // --- ADDED: ensure no missing arguments when parentheses are absent ---
  if (lookAhead->tokenType != SB_LPAR && paramList != NULL)
    error(ERR_PARAMETERS_ARGUMENTS_INCONSISTENCY, currentToken->lineNo, currentToken->colNo);
}

void compileCondition(void) {
  // TODO: check the type consistency of LHS and RSH, check the basic type
  // --- ADDED: ensure both sides basic and equal ---
...
Type* compileFactor(void) {
  ...
  case TK_NUMBER:
    eat(TK_NUMBER);
    type = makeIntType(); // --- ADDED: number yields int ---
    break;
  case TK_CHAR:
    eat(TK_CHAR);
    type = makeCharType(); // --- ADDED: char literal yields char ---
    break;
  case TK_IDENT:
    eat(TK_IDENT);
    ...
    switch (obj->kind) {
    case OBJ_CONSTANT:
      type = duplicateType(obj->constAttrs->value->type == TP_INT ? intType : charType); // --- ADDED: const type ---
      break;
    case OBJ_VARIABLE:
    ...
    case OBJ_PARAMETER:
      ...
      type = compileIndexes(type); // --- ADDED: allow indexing parameters ---
      break;
    case OBJ_FUNCTION:
